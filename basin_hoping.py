          ...
          if iter_todo > 0:
            if os.path.exists('accept.dat'):
              with open("accept.dat", "r") as file:
                ...
                N_min_e = last_accept_iter - last_iter
            else:
              N_min_e = 0
              E = 0.0
  ...
  for step, program in enumerate(programs[step_todo:]): 
      ...
      for N in range(1, program["prog_param"]["Nstep"]+1):
        ...
        output = vasp_tmp\
               (\
                 structure_tmp,\
                 outdir = join(outdir, join(str(step+step_todo), join("data", str(N+last_iter)+'_0' ) )),\
                 #relaxation = 'ionic',\
                 #relaxation = 'static',\
                 **params\
               )
        output = vasp_tmp.Extract(join(outdir, join(str(step+step_todo), join("data", str(N+last_iter) +'_0') )) )
